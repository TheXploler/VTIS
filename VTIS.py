...
class Main(baseui.MainUI):
    ...
    def on_video_drop(self, event):
        # Get file path from dnd
        video_file = event.data
        if os.path.isfile(video_file):
            self.VideoEntry.delete(0, tk.END)
            self.VideoEntry.insert(0, video_file)

    def on_output_dir_drop(self, event):
        # Get dir from dnd
        ...
        if os.path.isdir(dropped_path):
            self.OutputDirEntry.delete(0, tk.END)
            self.OutputDirEntry.insert(0, dropped_path)
        elif os.path.isfile(dropped_path):
            # Take parent dir if file is dropped
            dropped_dir = os.path.dirname(dropped_path)
            self.OutputDirEntry.delete(0, tk.END)
            self.OutputDirEntry.insert(0, dropped_dir)
    ...
    def extract_frames(self, video_path, output_dir, output_format):
        ...
        try:
            ...
            total_frames = int(subprocess.check_output(
                probe_cmd).decode().strip())
        except (subprocess.CalledProcessError, ValueError):
            messagebox.showerror(
                "Error", "Could not determine video frame count")
            return False
        ...
        def run_extraction():
            try:
                cmd = [
                    ...
                    '-start_number', '0',
                    os.path.join(
                        output_dir, f'frame_%05d.{output_format.lower()}')
                ]
                ...
                messagebox.showinfo(
                    "Success", f"Extracted {total_frames} frames to {output_dir}")
    ...
    def SP(self):  # Start Processing
        ...
        if not os.path.exists(video_path):
            messagebox.showerror(
                "Error", "Selected video file does not exist.")
            return

        if not os.path.isdir(output_dir):
            messagebox.showerror(
                "Error", "Selected output directory is invalid.")
            return
