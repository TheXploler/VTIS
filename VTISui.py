...
class MainUI:  
    def __init__(self, master=None):  
        ...
        # VE drop target  
        tk2.drop_target_register(DND_FILES)  
        tk2.dnd_bind('<<Drop>>', lambda event: self.on_video_drop(event))  

        self.OutputFormatLbl = ttk.Label(  
            tk2, text='Output Format', bootstyle="inverse-primary")  
        ...
        # OD drop target  
        self.OutputDirEntry.drop_target_register(DND_FILES)  
        self.OutputDirEntry.dnd_bind('<<Drop>>', lambda event: self.on_output_dir_drop(event))  
        ...
        self.ProcessBtn = ttk.Button(  
            tk2, text='Start Processing', bootstyle="success")  
        self.ProcessBtn.grid(column=0, columnspan=3, padx=10, pady=10, row=3)  
        self.ProcessBtn.configure(command=self.SP)  

        self.VideoInputLbl = ttk.Label(  
            tk2, text='Video Input', bootstyle="inverse-primary")  
        ...
        self.OutputDirLbl = ttk.Label(  
            tk2, text='Output Directory', bootstyle="inverse-primary")  
        self.OutputDirLbl.grid(column=0, padx=10, pady=10, row=2, sticky="w")  

        self.FormatCombobox = ttk.Combobox(  
            tk2, values=['JPG', 'PNG'], bootstyle="primary")  
